import { sendEmailVerificationMail, sentOTPEmail } from '../../service/comman/emailNew.repo.js';
...
export const SendEmailVerificationMail = async (req, res) => {
    try {
        const resp = await sendEmailVerificationMail(req.body);
        console.log(resp);
        return res.status(200).json({ success: true, message: resp });
    } catch (error) {
        console.log(error);
